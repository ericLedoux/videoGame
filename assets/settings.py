# Our settings go here (another python convention is all caps for global variables).  Change 'WIDTH' and 'HEIGHT' 
# variables to make the screen less/more wide or high.
WIDTH = 1000
HEIGHT = 800
# Frames per second (gamers rise up)
FPS = 30
PLAYER_FRIC = -.75
PLAYER_GRAV = 0
# Since both Mob and Death Block count are global, never-changing variables, it belongs in the settings
MOB_COUNT = 15
DEATH_BLOCK_COUNT = 10

# Next, we define colors (with RGB values of course).  Also, these colors are global so we write them in all caps.
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0, 0, 255)
LIGHT_GREY = (192, 192, 192)
DARK_GREY = (128, 128, 128)
YELLOW = (255, 255, 0)